<?php 
            ...
            if(!isset($_SESSION['login'])) {
                ...
                if(!empty($_POST)) {
                    $sql = 'UPDATE utilisateurs SET nom="'.$_POST['nom'].'" , prenom="'.$_POST['prenom'].'" , age='.$_POST['age'].' , pseudo="'.$_POST['pseudo'].'" , mail="'.$_POST['mail'].'" WHERE mail="'.$_SESSION['login'].'"';
                ...
                $sql = 'SELECT * FROM utilisateurs WHERE mail="'.$_SESSION['login'].'"';
                $query = $bdd->prepare($sql);
                $query->execute();
...
<form action="informations.php" method="POST">
    ...
    <h1 class="text-center title-top">Mes informations </h1>
        <div class="col-6">
                <div class="mb-3">
